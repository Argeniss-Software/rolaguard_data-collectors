...
from sqlalchemy import Column, DateTime, String, Integer, BigInteger, SmallInteger, Float, Boolean, ForeignKey, \
    func, asc, desc, func, LargeBinary, Enum as SQLEnum
...
class PotentialAppKey(Base):
    ...
    def find_all_by_organization_id_after_datetime(cls, organization_id, datetime):
        return session.query(cls).filter(cls.organization_id == organization_id, cls.last_seen > datetime).all()
